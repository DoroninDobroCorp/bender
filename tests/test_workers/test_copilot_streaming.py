"""Tests for CopilotWorker streaming functionality.

Tests GenericStreamingAdapter integration for copilot worker.
"""
...
class TestCopilotWorkerCreateAdapter:
    ...
    def test_create_streaming_adapter_returns_generic_adapter(self):
        ...
        worker = CopilotWorker(config=config)
    ...
    def test_adapter_can_feed_text(self):
        ...
        events = adapter.feed_text("Test output from copilot")

        assert len(events) == 2  # ITEM_START + ITEM_DELTA
        assert events[1].data["text"] == "Test output from copilot"
...
class TestCopilotWorkerAdapterIntegration:
    """Integration tests for CopilotWorker adapter usage."""

    def test_multiple_workers_have_independent_adapters(self):
        config = WorkerConfig(project_path=Path("/tmp/test"))
        worker1 = CopilotWorker(config=config)
        worker2 = CopilotWorker(config=config)
        ...
        adapter1.start_session(agent_id="copilot-w1")
        adapter2.start_session(agent_id="copilot-w2")

        # Each adapter tracks its own sequence
        events1 = adapter1.feed_text("Worker 1 text")
        events2 = adapter2.feed_text("Worker 2 text")

        assert events1[0].agent_id == "copilot-w1"
        assert events2[0].agent_id == "copilot-w2"
...
class TestCopilotWorkerAdapterSyntheticEvents:
